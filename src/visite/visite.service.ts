...
export class VisiteService {
  constructor(
    ...
    private readonly concurentService: ConcurentService, // Inject ConcurentService
  ...
  async create(ajouterVisiteDto: AjouterVisiteDto): Promise<Visite> {
    ...
    try {
      ...
      if (client) {
        const clientId = new Types.ObjectId(client); // Explicitly cast to ObjectId
  ...
  async findAllByClient(clientId: string): Promise<any[]> {
    const visites = await this.visiteModel
      ...
      .populate({
        ...
        select: 'nom prix', // Fetch details about the products
      })
      .exec();
